...
const PastTab = ({ data, isLoading }: IPendingPropsTab) => {
  if (isLoading) return <FallbackSpinner sx={{ height: '70vh' }} />
        ...
        <Card sx={{ boxShadow: 0, border, borderRadius: 1 }}>
          ...
          {data?.length === 0 ? (
            ...
            <Grid margin={4} sx={{ border, borderRadius: 1 }}>
              <TableCollapsible data={data || []} headerRow={tableHeaderRows} renderPickedDate />
            </Grid>
          )}
        </Card>
      </Grid>
